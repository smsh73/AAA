...
Analyst Awards System의 데이터베이스 마이그레이션을 실행하는 방법을 안내합니다.

## 최신 변경사항 (2025-01)

- **BackgroundTasks 도입**: Celery/Redis 의존성 제거 (선택사항)
- **Enum 정의**: 상태 관리를 위한 Enum 클래스 추가
...
- **003 (478d6db4f74b)**: Company 모델의 ticker 필드를 nullable로 변경 (자동 추출 기능 지원)
...
1. ✅ API 서버 재시작
2. ✅ Celery 워커 재시작 (선택사항 - BackgroundTasks 사용 시 불필요)
3. ✅ 통합 테스트 실행 (`pytest tests/`)
4. ✅ 기능 동작 확인
5. ✅ PDF 일괄 업로드 테스트 (선택사항)
6. ✅ 평가/리포트 관리 페이지 계층 구조 확인
7. ✅ 스코어카드 자동 생성 확인
8. ✅ 자료수집 및 KPI 평가 기능 테스트
9. ✅ 기업명 자동 추출 기능 테스트 (리포트 업로드 시 기업 선택 안 함)
10. ✅ 예측 정보 자동 추출 기능 테스트 (리포트 파싱 완료 후 예측 정보 확인)
11. ✅ 자동 데이터 수집 연동 테스트 (리포트 파싱 완료 후 데이터 수집 시작 확인)
...
- 리포트 파싱: BackgroundTasks로 자동 실행
- 평가 프로세스: Celery 또는 BackgroundTasks 사용
- Redis는 선택사항이며, 없어도 기본 기능은 정상 동작합니다

### Enum 정의

상태 관리를 위한 Enum 클래스가 추가되었습니다:
- `EvaluationStatus`: pending, processing, completed, failed
- `ReportStatus`: pending, processing, completed, failed
- `CollectionJobStatus`: pending, running, completed, failed, cancelled

### 외부 API 통합

- **OpenDART API**: 기업 실적 데이터 수집 (`DART_API_KEY` 환경변수 필요)
- **KRX API**: 주가 데이터 수집 (API 키 불필요)

### 정합성 개선 (2025-01 최신)

- **DataCollectionLog 스키마 보완**: 누락된 필드 추가 (`prompt_template_id`, `perplexity_request`, `perplexity_response`, `token_usage`)
...
- **평가/리포트 관리 페이지**: 기간>애널리스트>리포트 계층 구조 구현
- **애널리스트 삭제**: 관련 데이터 확인 후 삭제 (리포트, 평가, 스코어카드, 어워드)
