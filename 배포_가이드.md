...
**최신 변경사항 (2025-01-15)**:
- **마이그레이션 003**: Company 모델의 `ticker` 필드를 `nullable=True`로 변경 (자동 추출 기능 지원)
...
# Docker Compose 사용 시
docker-compose exec api python scripts/batch_upload_pdfs.py
```

**PDF 파일 위치**: `old_assets/Analyst PDF/` (하위 폴더 포함)
...
- `REDIS_URL`: Redis 연결 문자열 (선택사항 - BackgroundTasks 사용 시 불필요)
- `NEXT_PUBLIC_API_URL`: 프론트엔드에서 사용할 API URL
- `PERPLEXITY_API_KEY`: Perplexity API 키 (데이터 수집용)
- `OPENAI_API_KEY`: OpenAI API 키 (평가용)
- `ANTHROPIC_API_KEY`: Anthropic Claude API 키 (평가용)
- `GOOGLE_API_KEY`: Google Gemini API 키 (평가용)
- `DART_API_KEY`: OpenDART API 키 (기업 실적 데이터 수집용) - **신규 추가**
- `CORS_ORIGINS`: 허용할 프론트엔드 도메인

**환경변수 설정 예시**:
```bash
# .env 파일 또는 환경변수 관리 도구
DATABASE_URL=postgresql://user:password@localhost:5432/analyst_awards
REDIS_URL=redis://localhost:6379/0  # 선택사항
NEXT_PUBLIC_API_URL=https://api.yourdomain.com
PERPLEXITY_API_KEY=your_perplexity_key
OPENAI_API_KEY=your_openai_key
ANTHROPIC_API_KEY=your_anthropic_key
GOOGLE_API_KEY=your_google_key
DART_API_KEY=your_dart_key  # OpenDART API 키 발급 필요
CORS_ORIGINS=https://yourdomain.com,https://www.yourdomain.com
```
...
2. 회원가입 및 로그인
3. API 인증키 발급
4. 발급받은 키를 `DART_API_KEY` 환경변수에 설정
...
# 마이그레이션 버전 확인 (003까지 적용되어야 함)
alembic history
```

**현재 마이그레이션 버전**:
- **001**: 초기 테이블 생성 (18개 테이블)
- **002**: collection_jobs 테이블 추가 및 data_collection_logs 업데이트
- **003 (478d6db4f74b)**: Company 모델의 ticker 필드를 nullable로 변경
...
**참고: BackgroundTasks 사용**
- 현재 시스템은 FastAPI BackgroundTasks를 사용하여 비동기 작업을 처리합니다
- Redis/Celery는 선택사항이며, BackgroundTasks가 우선적으로 사용됩니다
- Redis가 없어도 리포트 파싱, 평가 등이 정상 동작합니다
- 프로덕션 환경에서 대용량 처리가 필요한 경우 Celery 사용을 권장합니다
...
- [ ] 환경 변수 설정 완료
- [ ] 데이터베이스 연결 확인
- [ ] Redis 연결 확인 (선택사항 - BackgroundTasks 사용 시 불필요)
- [ ] API 키 설정 확인 (Perplexity, OpenAI, Anthropic, Google, DART)
- [ ] CORS 설정 확인
- [ ] 포트 충돌 확인
- [ ] OpenDART API 키 발급 및 설정 (기업 실적 데이터 수집용)
...
- [ ] 백엔드 서버 시작
- [ ] Celery Worker 시작 (선택사항 - BackgroundTasks 사용 시 불필요)
- [ ] 프론트엔드 빌드 (프로덕션)
- [ ] 프론트엔드 서버 시작
- [ ] PDF 일괄 업로드 실행 (선택사항)
- [ ] 정합성 점검: 자료수집 및 KPI 평가 기능 확인
...
- [ ] 샘플 데이터 생성 (개발 환경만)
- [ ] 리포트 업로드 및 파싱 테스트
- [ ] 평가 프로세스 테스트 (OpenDART/KRX API 연동 확인)
- [ ] 에이전트 콘솔 동작 확인
- [ ] 평가/리포트 관리 페이지 계층 구조 확인
- [ ] 스코어카드 자동 생성 확인 (평가 완료 후)
- [ ] 자료수집 기능 테스트 (SNS/미디어 데이터 수집)
- [ ] KPI 점수 계산 확인 (7개 KPI 모두 포함)
- [ ] 애널리스트 삭제 기능 테스트 (관련 데이터 확인)
...
- [ ] OpenDART API 키 보안 관리
- [ ] 외부 API 호출 시 타임아웃 설정 확인

## 11. 주요 변경사항 (최신 업데이트)

### 11.1 BackgroundTasks 도입

- FastAPI BackgroundTasks를 사용하여 비동기 작업 처리
- Celery/Redis 의존성 제거 (선택사항)
- 배포 단순화 및 세션 충돌 방지
...
### 11.6 정합성 점검 및 보완 (2025-01 최신)

#### 자료수집 기능 개선
- **DataCollectionLog 스키마 보완**: 누락된 필드 추가
  - `prompt_template_id`: 프롬프트 템플릿 ID
  - `perplexity_request`: Perplexity 요청 데이터
  - `perplexity_response`: Perplexity 응답 데이터
  - `token_usage`: 토큰 사용량 정보
- **프론트엔드 타입 안정성**: `overall_progress` 타입 개선 (number | string)

#### KPI 평가 기능 개선
- **SNS 주목도 및 미디어 언급 빈도**: 실제 DataCollectionLog 데이터 기반 계산
  - 최근 3개월 데이터 조회
  - 수집된 데이터에서 주목도 지표 추출
  - 언급 빈도 기반 점수 계산
- **스코어카드 생성 시 KPI 점수**: 7개 KPI 점수 모두 포함
  - `target_price_accuracy`: 목표주가 정확도 (25%)
  - `performance_accuracy`: 실적 추정 정확도 (30%)
  - `investment_logic_validity`: 투자 논리 타당성 (15%)
  - `risk_analysis_appropriateness`: 리스크 분석 적정성 (10%)
  - `report_frequency`: 리포트 발행 빈도 (5%)
  - `sns_attention`: SNS 주목도 (10%)
  - `media_frequency`: 미디어 언급 빈도 (5%)
- **평가 완료 후 스코어카드 자동 생성**: `evaluate_async()` 완료 시 자동 생성
- **평가 태스크 구현**: Celery 태스크에서 실제 평가 실행 로직 구현
- **SNS·시장 반응 점수 계산**: 실제 데이터 기반 계산으로 개선
...
- **애널리스트 삭제**: 관련 데이터 확인 후 삭제
  - 관련 리포트, 평가, 스코어카드, 어워드 확인
  - 관련 데이터가 있으면 삭제 불가 및 상세 메시지 표시
  - 데이터 수집 로그 및 작업은 자동 삭제
...
5. 서비스 시작
6. 정합성 확인 (평가/리포트 관리 페이지, 스코어카드 자동 생성)
...
2. **데이터 정합성 검증**
   - 애널리스트 삭제 시 관련 데이터 확인
   - 스코어카드에 모든 KPI 점수 포함 확인
   - 평가 완료 후 스코어카드 자동 생성 확인

3. **API 정합성 검증**
   - DataCollectionLog API 응답에 모든 필드 포함 확인
   - 평가 점수 API 응답 확인
   - 스코어카드 API 응답 확인
